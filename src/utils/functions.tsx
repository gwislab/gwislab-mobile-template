...
export const getAge = (dateString: Date) => {
  return getMoment(dateString).isValid()
    ? getMoment().diff(dateString, "years")
    : "--";
};
...
export const formatDateMoment = (dateString: Date, format?: string) => {
  return getMoment(dateString).isValid()
    ? capitalizeFirstLetter(
        getMoment(dateString).format(format || "DD MMM YYYY")
      )
    : "--/--/--"; // 03 jan 2923
};
...
export const getAllDaysInMonth = (month: number, year: number, key: any) => {
  const dates = Array.from(
    ...
    (_, i) => getMomentDate([year, month - 1, i + 1])
  );
...
export const convertDataToKeyValue = (
  data: any[] = [],
  key?: string
): any[] => {
...
export const getDayOfWeek = (
  ...
  const result = isNaN(dayOfWeek)
    ? null
    : capitalizeFirstLetter(daysOfWeek[dayOfWeek]);
